...
\subsection{Automated Virtual Cameras}
Creating a system that makes the camera animation automatically for a game can be beneficial at both a professional level to reduce cost or at an amateur level to increase quality. Systems that emphazises automatic behavior has been implemented before \cite{bourne_constraintBased_2008, burelli_automatic_2014}. These systems uses an intelligent approach whereas the FCT will be driven by artists that want to specify each camera animation themselves.
